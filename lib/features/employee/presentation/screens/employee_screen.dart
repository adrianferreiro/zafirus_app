  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: state.when(
        ...
        loaded: (employee) => _EmployeeDetail(employee: employee),
      ),
    );
  }
}
      ...
      children: [
        ...
        _Section(title: 'Información Personal', children: [
          ...
          if (employee.personalEmail != null)
            _InfoTile(icon: Icons.alternate_email, label: 'Email personal', value: employee.personalEmail!),
          if (employee.dateOfBirth != null)
            _InfoTile(icon: Icons.cake, label: 'Fecha de nacimiento', value: employee.dateOfBirth!),
          if (employee.gender != null)
            _InfoTile(icon: Icons.person, label: 'Género', value: employee.gender!.name),
        ]),
        _Section(title: 'Información Laboral', children: [
          if (employee.department != null)
            _InfoTile(icon: Icons.business, label: 'Departamento', value: employee.department!.name),
          if (employee.division != null)
            _InfoTile(icon: Icons.account_tree, label: 'División', value: employee.division!.name),
          if (employee.jobLevel != null)
            _InfoTile(icon: Icons.trending_up, label: 'Nivel', value: employee.jobLevel!.name),
          if (employee.employmentType != null)
            _InfoTile(icon: Icons.work, label: 'Tipo de empleo', value: employee.employmentType!.name),
          if (employee.location != null)
            _InfoTile(icon: Icons.location_on, label: 'Ubicación', value: employee.location!.name),
          if (employee.hiredOn != null)
            _InfoTile(icon: Icons.calendar_today, label: 'Fecha de ingreso', value: employee.hiredOn!),
        ]),
        if (employee.reportingTo != null)
          _Section(title: 'Reporta a', children: [
            _InfoTile(
              icon: Icons.supervisor_account,
              label: '${employee.reportingTo!.firstName} ${employee.reportingTo!.lastName}',
              value: employee.reportingTo!.email,
            ),
          ]),
        if (employee.directReports.isNotEmpty)
          _Section(title: 'Reportes directos', children: [
